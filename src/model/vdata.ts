 ...
 *  this._proxyViewdata = this._viewModel.init({
        gridmap: this._computedgridmap,   // 基础网格数据
        cellmm: this._initcellmm,         // 单元格信息
        scrollIdexes: this._scrollIdexes, // 控制滚动
    });
 * 
 */
...
export class ViewModel {
    ...
    init(viewdataObj: ViewDataSource): ViewDataSource {
        ...
        // 首次需要手动调用render
        this._updateCanvasView();
        return this.state;
    }
    // 首屏渲染前，对当前数据进行代理，然后渲染，得到依赖收集，
    // 同时数据代理方便 scroll数据的 viewdata控制
    proxy<T extends Array<T>>(target: ViewDataSource, prefixKey?: PropertyKey): ViewDataSource {
        ...
        observed = new Proxy(target, {
            get: (target: ViewDataSource, key: PropertyKey) => {
                // let curkey = key;
                // curkey = (prefixKey && prefixKey + '.') + key;
